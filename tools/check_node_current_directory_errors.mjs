...
const temporary = await mkdtemp(join(tmpdir(), "lnako-node-cwd-errors-"));

try {
  const source = resolve(temporary, "case.nako3");
  await writeFile(source, [
    'エラー監視',
    'カレントディレクトリ変更("missing-日本語-7f4b")',
    "エラーならば",
    "エラーメッセージを表示",
    "ここまで",
    "",
  ...
  const expected = results.official.stdout;
  const failures = Object.entries(results).filter(([, result]) => result.status !== 0 || result.stdout !== expected);
  if (failures.length > 0) {
    throw new Error(`Node cwd error差分: ${JSON.stringify(results, null, 2)}`);
  }
  console.log("Nodeカレントディレクトリ失敗診断差分テスト: 公式CLI・Interpreter・AOT O0〜O3成功");
} finally {
...
function normalize(result) {
  return { status: result.status, stdout: result.stdout.replaceAll("\r\n", "\n"), stderr: result.stderr };
}
