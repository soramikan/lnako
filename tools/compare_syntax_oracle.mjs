...
const cases = JSON.parse(await readFile(resolve(root, "tests/oracle/syntax-transform-cases.json"), "utf8"));
const [{ NakoTokenizer }, { NakoLogger }] = await Promise.all([
  import(pathToFileURL(resolve(oracleRoot, "core/src/nako_tokenizer.mjs"))),
  import(pathToFileURL(resolve(oracleRoot, "core/src/nako_logger.mjs"))),
]);

const probe = spawnSync("zig", ["build", "syntax-probe", "--", ...cases], {
  cwd: root,
  encoding: "utf8",
  env: { ...process.env, ZIG_GLOBAL_CACHE_DIR: process.env.ZIG_GLOBAL_CACHE_DIR ?? resolve(root, ".zig-global-cache") },
  maxBuffer: 16 * 1024 * 1024,
});
if (probe.status !== 0) throw new Error(`syntax-probe失敗:\n${probe.stderr}`);
const actualCases = probe.stdout.trimEnd().split("\n").map((line) => JSON.parse(line));

const kindMap = new Map([
  ["word", "identifier"], ["string", "string"], ["number", "number"], ["bigint", "bigint"],
  ["eol", "eol"], ["comma", "comma"], ["もし", "keyword_if"], ["違えば", "keyword_else"],
  ["ここから", "keyword_here_from"], ["ここまで", "keyword_here_end"], ["def_func", "def_func"],
  ["def_test", "def_test"], ["func", "function_ref"], ["eq", "equal"], ["===", "strict_equal"],
  ["noteq", "not_equal"], ["!==", "strict_not_equal"], ["gt", "greater"], ["gteq", "greater_equal"],
  ["lt", "less"], ["lteq", "less_equal"], ["and", "logical_and"], ["or", "logical_or"], ["not", "not"],
  ["+", "plus"], ["-", "minus"], ["*", "multiply"], ["**", "power"], ["÷", "divide"], ["÷÷", "integer_divide"],
  ["%", "modulo"], ["^", "bit_xor"], ["&", "bit_and"], ["@", "at"], ["$", "property"], ["??", "question_display"],
...
function normalizeOfficial(token) {
  const value = String(token.value ?? "");
  const kind = token.type === "word" && reservedMap.has(value) ? reservedMap.get(value) : kindMap.get(token.type);
  if (!kind) throw new Error(`未対応の公式トークン種別: ${token.type}:${value}`);
  return { kind, value: token.type === "eol" ? "" : value, josi: token.josi ?? "", indent: token.indent ?? 0 };
}
function normalizeActual(token) {
  return {
    kind: token.kind,
    ...
    .flatMap((token) => {
      if (token.type === "word" && token.josi === "" && String(token.value).endsWith("回") && String(token.value).length > 1) {
        return [
          ...
          { kind: "keyword_repeat_count", value: "回", josi: "", indent: token.indent ?? 0 },
        ];
      }
...
if (failures > 0) throw new Error(`構文変換の差分が${failures}件あります`);
console.log(`公式v3.7.24とのインデント・DNCL差分テスト: ${cases.length}件成功`);
