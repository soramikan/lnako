...
const categories = new Set(["ファイル入出力", "パス操作", "フォルダ取得", "環境変数", "圧縮・解凍", "Nodeプロセス", "コマンドラインと標準入出力", "文字コード"]);
const expected = catalog.commands.filter((command) => command.plugin === "plugin_node" && categories.has(command.category)).map((command) => command.name).sort();
const mappings = new Map();

for (const path of ["tests/oracle/node-file-cases.json", "tests/oracle/node-exit-cases.json", "tests/oracle/node-native-cases.json"]) {
  for (const testCase of JSON.parse(await readFile(resolve(root, path), "utf8"))) for (const name of testCase.commands) add(name, testCase.id);
}
...
console.log(`Nodeファイル・パス・プロセス・文字コード・圧縮命令カバレッジ: ${expected.length}/${expected.length}`);
