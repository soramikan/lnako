...
const lnako_aot_ajax_onerror_set = control_flow.lnako_aot_ajax_onerror_set;
const lnako_aot_ajax_options_set = control_flow.lnako_aot_ajax_options_set;
const lnako_aot_archive_call = control_flow.lnako_aot_archive_call;
const lnako_aot_archive_tool_path_set = control_flow.lnako_aot_archive_tool_path_set;
...
const lnako_aot_debug_display = debug.lnako_aot_debug_display;
const lnako_aot_destructure_get = state.lnako_aot_destructure_get;
const lnako_aot_dispatch_display_begin = debug.lnako_aot_dispatch_display_begin;
const lnako_aot_dispatch_display_begin_with_epoch = debug.lnako_aot_dispatch_display_begin_with_epoch;
const lnako_aot_dispatch_result = debug.lnako_aot_dispatch_result;
const lnako_aot_exception_pending = debug.lnako_aot_exception_pending;
const lnako_aot_exception_set = debug.lnako_aot_exception_set;
const lnako_aot_exception_set_error_message = debug.lnako_aot_exception_set_error_message;
const lnako_aot_exception_take = debug.lnako_aot_exception_take;
...
const lnako_aot_promise_call_site = control_flow.lnako_aot_promise_call_site;
const lnako_aot_regexp_call_site = debug.lnako_aot_regexp_call_site;
const lnako_aot_runtime_drain_events = control_flow.lnako_aot_runtime_drain_events;
const lnako_aot_shift = state.lnako_aot_shift;
const lnako_aot_stdio_call = debug.lnako_aot_stdio_call;
const lnako_aot_timer_call_site = control_flow.lnako_aot_timer_call_site;
...
test "AOT反復構文は「それ」と指定変数へ束縛し指定変数ありで「対象」を更新しない" {
    // Issue #112: 公式convForeach互換。生成コードは「対象」・「対象キー」・
    // ループ変数・「それ」の4ポインタを渡し、指定変数がある`AをBで反復`は
    // 「対象」ポインタをnullにする。数値の反復対象は0回実行。
    var runtime = Runtime{ .allocator = std.testing.allocator };
    defer runtime.deinit();
    var values = [_]Value{try runtime.createArray(&.{ numberValue(3), numberValue(4) })};
    var frame: RootFrame = .{};
    ...
    // `AをBで反復`: 指定変数へ束縛し「対象」はnullで更新されない。
    var variable: Value = .{};
    taisyou = .{};
...
test "AOT低レイヤーの未実装命令は全てstub経由でENOTSUPを返す" {
    ...
    try std.testing.expectEqual(@as(usize, 4), stub_count);
}
