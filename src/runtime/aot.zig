...
test "AOT __DEBUGはデバッグ状態を有効化して値を返さない" {
    ...
    try std.testing.expect(active_runtime.?.debug_enabled);
...
test "AOT漢数字命令は指数・全角数字・小数・BigIntを処理する" {
    ...
    roots[2] = try runtimeUtf8String(&active_runtime.?, "0.01");
    ...
    roots[4] = try runtimeUtf8String(&active_runtime.?, "1e-3");
    ...
    roots[6] = try runtimeUtf8String(&active_runtime.?, "１２３");
    ...
    roots[8] = try runtimeUtf8String(&active_runtime.?, "一万一");
    ...
    roots[0] = try runtimeUtf8String(&active_runtime.?, "一・二");
    ...
    roots[2] = try runtimeUtf8String(&active_runtime.?, "一無量大数");
...
test "AOT AJAXオプション設定は設定値をグローバルへ保持する" {
    ...
    roots[0] = try active_runtime.?.createString(&.{});
    roots[1] = try runtimeUtf8String(&active_runtime.?, "{\"method\":\"PUT\",\"body\":\"payload\"}");
    roots[2] = try jsonDecodeBuiltin(&active_runtime.?, roots[1]);
