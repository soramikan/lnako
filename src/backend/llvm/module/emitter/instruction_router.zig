...
pub fn writeInstruction(emitter: *Emitter, function: ir.Function, locals: []const []const u8, instruction: ir.Instruction, scope: usize, aggregate_count: usize) !void {
    ...
    switch (instruction.opcode) {
        ...
        .is_array => try collections_mod.writeIsArray(emitter, instruction, scope),
        .init_array_index => try collections_mod.writeInitArrayIndex(emitter, instruction, scope),
        .is_undefined => try variables_mod.writeIsUndefined(emitter, instruction, scope),
        .coalesce_or_zero => try variables_mod.writeCoalesceOrZero(emitter, instruction, scope),
        .increment_values => try variables_mod.writeIncrementValues(emitter, instruction, scope),
