...
pub const commands = @import("parser/commands.zig");
...
pub const Parser = struct {
    ...
    pub fn parseStatement(self: *Parser) ParseFailure!*ast.Node {
        ...
        return switch (token.kind) {
            ...
            .keyword_here_end, .keyword_else, .keyword_error => self.fail(.unexpected_token, "対応する構文の開始がありません", token),
                ...
                // 連文の末尾が条件助詞付きの命令呼出しの場合も、公式と同じく
                // 末尾の呼出しを条件文へ昇格させる
                // （`XしてAが1と等しいならば戻る`は`block[X, if(等しい){戻る}]`）。
                if (statement.kind == .block and statement.children.len > 0) {
                    const last = statement.children[statement.children.len - 1];
                    if (last.kind == .function_call and isConditionalJosi(last.josi) and
                        (last.is_c_style_call or
                            ((last.command_call or last.children.len == 0) and self.isKnownCommandName(last.name))))
                    {
                        const if_node = try self.parseIfThen(token, try self.finishCondition(last));
                        const children = try self.allocator.alloc(*ast.Node, statement.children.len);
                        @memcpy(children[0 .. children.len - 1], statement.children[0 .. statement.children.len - 1]);
                        children[children.len - 1] = if_node;
                        break :blk try builder.makeNodeWithChildren(self, .block, token, children);
                    }
                }
                break :blk statement;
            },
    ...
    /// （代入/定める/増やす/減らす）の構築は`parser/commands.zig`へ移譲する。
    /// `self.X()`呼出しは公開const経由で従来どおり解決される。
    pub const finishChained = commands.finishChained;
    pub const rangeArguments = commands.rangeArguments;
    pub const resolveCommandName = commands.resolveCommandName;
    pub const isBuiltinCommandName = commands.isBuiltinCommandName;
    pub const isKnownCommandName = commands.isKnownCommandName;
    pub const isChainedBuiltinCommand = commands.isChainedBuiltinCommand;
    pub const callChainedBuiltinCommand = commands.callChainedBuiltinCommand;
    pub const makeCommandCall = commands.makeCommandCall;
    pub const parseImplicitCallbackCall = commands.parseImplicitCallbackCall;
    pub const implicitIt = commands.implicitIt;
    pub const parseJapaneseCommand = commands.parseJapaneseCommand;
    ...
    pub fn parseCallExpression(self: *Parser) ParseFailure!*ast.Node {
        ...
        while (true) {
            if (self.at(.identifier)) {
                ...
                if (self.peek().josi.len > 0 and !isConditionalJosi(self.peek().josi) and
