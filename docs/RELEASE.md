...
- 現在、`v0.2.2` が最新リリースです（過去のリリース: `v0.1.0`、`v0.1.1`、`v0.2.0`、`v0.2.1`）。
...
1. **バージョンの同期**：
   - `build.zig.zon` の `.version` と `src/root.zig` の `pub const version` を更新対象バージョン（例: `0.2.2`）へ揃えます（`lnako --version` が一致すること）。
2. **互換性証拠の再生成**：
   - ソース変更に伴いsource manifestが更新されるため、`node tools/sync_compat_evidence.mjs` を実行して証拠を現行manifestに同期します。
3. **CI全job成功の確認**：
   - mainブランチへ取り込み、CI（40 matrix job＋変更分類・軽量検証・Windows／Linux compiler producer・後段3 job、**合計47 job**）がfull相当ですべて成功するのを待ちます。
...
5. **署名済みタグの作成とpush**：
   - CIが成功した最終コミットに対し、GPG署名付きannotated tag（例: `git tag -s v0.2.2 -m "v0.2.2"`) を作成してpushします。
...
node tools/check_distribution.mjs \
  --archive /absolute/path/dist-standard/lnako-0.2.2-macos-arm64.tar.gz
node tools/check_distribution.mjs \
  --archive /absolute/path/dist-full/lnako-0.2.2-macos-arm64-full.tar.gz
```
