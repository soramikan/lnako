...
main CIの `attest-dispatch-evidence` jobが3 OSのdispatch証拠・native AOT aggregate・source manifest宣言・canonical証拠17件を同一Sigstore bundleのsubjectとして署名し、GitHub Attestationsへ記録します。Release tag pushは同じcommitのCI 56 job構成でのfull相当成功に加え、このattestationを `gh attestation verify` で再確認します。`compat/v3.7.24/attestations/` の過去snapshotはオフライン改変検査用の履歴fixtureであり、現行のverified判定には使いません。
