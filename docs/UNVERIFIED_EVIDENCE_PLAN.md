...
CIはjob増加とmacOS同時実行上限を考慮し、通常10 job、Linux/Windows専用parser fuzz 2 job、native AOT 27 job、Linux/Windows support AOT 12 jobの合計51 matrix job、coverage shard検証job、attestation jobへ分割した。macOSは1 runあたり5 jobを維持し、3正式OSのdispatch coverageを各3 weighted shardへ分ける。macOSでは既存native route 3 jobへcoverageを分散し、dispatch evidence／securityは`mac-host-compat`へ置く。直近完了run `33735104547`ではcoverageを`mac-core-standard-support`へ移しただけでは壁時計が前回より約1分伸びたため、次の構成でmacOS coverageを3 native route jobへ再配置する。これは証拠生成物のpush順序の問題ではなく、監査のjob割当によるcritical pathの問題である。検証範囲・AOT実行・macOS 5枠は削減せず、次回push前にも完了済み失敗runを再確認する。壁時計・runner合計の改善値は成功した完了runで測定するまで未確定とする。待機中に作業は停止しない。
