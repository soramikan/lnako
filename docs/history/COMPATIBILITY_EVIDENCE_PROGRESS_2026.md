...
> 証拠化の進捗履歴です。現在のstateと正本は [`../COMPATIBILITY_EVIDENCE.md`](../COMPATIBILITY_EVIDENCE.md) を参照してください。
...
`dispatch-evidence.json`はcleanな現行HEAD `c1f104c6afc3aa367c325757a1dec85b2f6218e4`から再生成したcanonical fixtureのInterpreter 944 event、Node route 42 event、AOT manifest 946件・runtime 1,888 eventを記録します。`dispatch-coverage-evidence.json`も同じclean commitから`--include-native`を含めて再生成し、227 fixture・4,489 site・426 native entry（492 unique names中424）を記録し、97 native entryを`unobservedNativeEntryIds`へ残します。これは成功経路・明示した期待失敗／host gap・同名命令のcatalog IDを含むunattested sampled coverageであり、全527 entryの純LLVM AOT実行証明ではありません。expected-exit、global/static constant、compat-jsの補助artifactも同じ現行HEAD provenanceへ更新しています。U28のparser修正後も公式AST差分84件、文法生成fuzz 8,221件（seed `20260903`）と別seed 8,221件（`20260904`）を成功させています。fixture inventoryは全414件（AOT 312件、Interpreter 112件、QuickJS 9件）です。system-onlyの公式生成JavaScriptはstandalone system plugin runtime bundleがないため実行不能として記録し、公式sourceを選択oracleにしています。

native AOTの全件性は、dispatch証拠とは別の`lnako.native-aot-aggregate-evidence.v1`で検証します。`tools/check_native_aot_artifacts.mjs`は`tests/oracle/native-cases.json`の292 fixture（451 command name）を、Linux x86_64 12 artifact、macOS arm64 3 artifact、Windows x86_64 12 artifactへ固定partitionし、各artifactの公式source・公式generated・`lnako run`・選択したLLVM AOT route、O0〜O3のcompile status、O0 manifest、結果hash、公式baseline、toolchain、source commitを再検証します。artifactの存在、native分類、fixture参照、単一OSの成功だけでは全件AOT証拠とみなしません。CIの`verify_native_aot_artifacts`が27 artifactを全件検査して生成したaggregateだけを、`verify_native_aot_attestation.mjs`が3正式OSのsource commit・main workflow・GitHub-hosted runnerへ公式`gh attestation verify`で拘束します。現行HEADにはこのCI runのaggregate外部署名証拠がまだ取り込まれていないため、catalogの`verified`昇格や527 entryの純LLVM AOT完了とは扱いません。
...
この文書の後続段落にはU07〜U26の完了時点を記録した履歴値と、U22前のfixture inventory／execution evidenceの説明が含まれます。現行値はこのスナップショットと`compat/v3.7.24/evidence.json`、各証拠artifactを正本とし、履歴段落の旧件数を現在値として解釈しません。
...
今回のWindows path回帰修正では、`native-node-path-mixed-separators`の公式source／generated／AOT一致に対してInterpreterだけが異なることをCI run `33708609448`で確認した。`src/plugins/node.zig`をAOTと同じNode 24相当のroot scanへ揃え、cleanな`f07e30d320339611730d3e563700a78b507a376d`からcanonical／coverageと全補助artifactを再生成した。公式仕様、差分原因、意図的制限ではないこと、Windows runnerの外部署名attestationが未完了であることは`docs/COMPATIBILITY_QUIRKS.md`へ分離して記録する。
...
以下の節にある「現行HEAD」や件数は各節の完了時点の履歴値であり、現在の台帳値ではありません。
...
現行HEADでは`verified: 0`、`trace-confirmed-unattested: 489`、`unverified: 38`です。U05の`native-node-directory-values`、U06の`plugin-node-native-archive-hermetic`、U07の`node-exit-evidence`、U08の`plugin-node-stdin`／`plugin-node-stdin-callback`、U09のnetwork topology証拠、U10の`plugin-node-http-callbacks`／`plugin-node-http-onerror`、U11の`plugin-node-http-options-and-promises`、U12の`plugin-node-http-async-values`、U13のDiscord loopback証拠、U15のHTTP server dispatch証拠を反映しています。U05はOS依存値を固定文字列にせず、`デスクトップ`・`マイドキュメント`・`テンポラリフォルダ`のglobal-read 3 siteを`directory-binding-evidence.json`へcatalog ID別に記録し、U07は終了コード・SIGINT callback・terminal traceを`expected-exit-evidence.json`へcatalog ID別に記録します。U08はraw stdin、EOF部分行、同期callbackを`dispatch-coverage-evidence.json`へcatalog ID別に記録します。U09は`自分IPアドレス取得`・`自分IPV6アドレス取得`のnetwork topologyと列挙順を同じ証拠へcatalog ID別に記録します。U10はloopback HTTPを使い、6 entryのsuccess/failure、`対象`更新、callback順序を同じ証拠へcatalog ID別に記録します。U11はPromise応答7命令の成功resolve、`AJAX内容取得`、`AJAX受信`の`対象`更新、event drainを同じ証拠へcatalog ID別に記録します。U12は5つのasync値命令について、text／JSON／空JSON／form／ArrayBufferの成功値を同じ証拠へcatalog ID別に記録します。U13は`DISCORD送信`のJSON success／HTTP 400期待失敗と`DISCORDファイル送信`のmultipart `content`／`file`を3 site（success 2、expected failure 1）として同じ証拠へcatalog ID別に記録します。U15は6 entryのHTTP server dispatchを、外部loopback 7 requestの応答hash、Interpreter/AOT trace、O0 compile manifestとともに同じ証拠へcatalog ID別に記録します。
別ファイルの`compat/v3.7.24/dispatch-coverage-evidence.json`は、canonical dispatch証拠とは分離したサンプル監査です。49 fixture・1,849 siteの成功経路と明示した期待失敗経路から、一意名として同定できる359 native entryを記録します。U06の4 entry（`解凍`、`解凍時`、`圧縮`、`圧縮時`）は、公式source・公式生成JavaScript・lnako Interpreter・AOT O0〜O3をfixture専用stored-ZIP helperで比較し、ZIP entry名・ディレクトリ・サイズ・CRC32・内容hashの意味結果を同一siteへ接続します。U08の4 entry（`標準入力取得時`、`尋`、`文字尋`、`標準入力全取得`）は、raw stdin、CRLF、EOF時の末尾改行なし部分行、同期callback drainを同じsiteで比較します。U10の6 entry（`AJAX送信時`、`AJAX受信時`、`GET送信時`、`POST送信時`、`POSTフォーム送信時`、`AJAX失敗時`）は、`plugin-node-http-callbacks`と`plugin-node-http-onerror`のloopback fixtureでsuccess/failure、`対象`更新、callback順序、AOT O0の同一siteを比較します。U11の7 entry（`AJAX保障送信`、`HTTP保障取得`、`GET保障送信`、`POST保障送信`、`POSTフォーム保障送信`、`AJAX内容取得`、`AJAX受信`）は、`plugin-node-http-options-and-promises`のloopback fixtureでPromise成功resolve、`対象`更新、event drain、AOT O0の同一siteを比較します。U12の5 entry（`POST送信`、`POSTフォーム送信`、`AJAXテキスト取得`、`AJAX_JSON取得`、`AJAXバイナリ取得`）は、`plugin-node-http-async-values`の22 dispatch siteで、公式source・公式生成JavaScript・`lnako run`・AOT O0の成功値を比較します。`AJAX_JSON取得`は通常bodyと空bodyの2 site、`AJAXバイナリ取得`はArrayBuffer形状を含みます。U13の`plugin-node-http-discord`、`plugin-node-http-discord-file`、`plugin-node-http-discord-failure`は、`DISCORD送信`のJSON success／HTTP 400期待失敗と`DISCORDファイル送信`のmultipart `content`／`file`をloopbackで比較し、3 site（success 2、expected failure 1）を接続します。U15のHTTP server fixtureは、6 entryのdispatchを外部loopback 7 requestで実行し、公式source・lnako Interpreter・AOT O0のstatus・header・body response hash、Interpreter/AOT trace、O0 compile manifestを同一fixtureへ接続します。公式生成JavaScriptはshutdown補助命令`終了`のstandalone plugin登録不足で利用不能な既知route差として記録します。これは公式source・`lnako run`・AOT O0の一致、Interpreter/AOT trace、compile manifestを同一siteで確認した`trace-confirmed-unattested`であり、raw ZIP byte列、任意の外部7z実装、外部HTTP endpoint、外部Discord endpoint、公式生成JavaScriptのstandalone plugin登録差、3 OS attestation、全527 entryのAOT実行を証明しません。`plugin-node-http-callbacks`の公式source／生成route差は`officialRoutesEquivalent: false`としてartifactに残し、公式sourceを選択oracleにしています。詳細な公式実装の出典と、説明不足・不具合候補・未実装境界の区別は`docs/COMPATIBILITY_QUIRKS.md`へ記録します。`native-node-network-addresses`のU09では、公式の[plugin_node命令一覧](https://nadesi.com/v3/doc/index.php?plugin_node=&show=)と固定upstreamの[`plugin_node.mts`](https://github.com/kujirahand/nadesiko3/blob/aa18c7e640523938c680958fe731418cc6f7a58f/src/plugin_node.mts#L1180-L1219)が使う`os.networkInterfaces()`の形状を、内部アドレスとIPv6の`scopeid`を含む`synthetic-v1` topologyとしてfixture専用に供給する。IPv4は`127.0.0.1`→`192.0.2.10`、IPv6は`::1`→`fe80::1234`→`2001:db8::10`の順を比較し、戻り値はアドレス文字列へ射影する。公式source・生成JavaScript・Interpreter・AOT O0の同一site、focused native shardのO0〜O3を確認済みだが、実OSの列挙順と3正式OSの外部署名attestationは証明しない。

初期89件の精査結果、実装単位、証拠基盤の依存関係、各単位の完了条件は[`docs/UNVERIFIED_EVIDENCE_PLAN.md`](UNVERIFIED_EVIDENCE_PLAN.md)を実行計画の正本とする。現行`evidence.json`では`unverified`は0件だが、これは全527 entryの純LLVM AOT実行や3正式OSの外部署名attestationを意味しない。U01の2 entryは期待失敗を含むdispatch evidence、U02の2 entryはdynamic-executeのdispatch evidence、U03の2 entryはOS別launcher経路と明示した期待失敗を含むdispatch evidence、U04の1 entryは可変globalのread/write site、U05の3 entryはOS依存値を固定しないglobal read site、U06の4 entryはhermetic archive helperによる本体・callbackのdispatch site、U07の3 entryは終了コード・SIGINT callback・terminal traceを専用artifactへ固定し、U08の4 entryはraw stdin・EOF部分行・同期callbackのdispatch siteを専用fixtureへ固定し、U09の2 entryは決定的network topologyのIPv4/IPv6 dispatch siteを専用fixtureへ固定し、U10の6 entryはloopback HTTPのsuccess/failure・`対象`更新・callback順序のdispatch siteを専用fixtureへ固定し、U11の7 entryはloopback HTTPのPromise成功resolve・`対象`更新・event drainのdispatch siteを専用fixtureへ固定し、U12の5 entryはloopback HTTPのtext／JSON／空JSON／form／ArrayBuffer成功値のdispatch siteを専用fixtureへ固定し、U13の2 entryはloopback HTTPのJSON／multipart／HTTP 400期待失敗のdispatch siteを専用fixtureへ固定して、いずれも`trace-confirmed-unattested`へ接続済みである。
追跡中の証拠は、U07完了時点の履歴内訳として、macOS arm64の単一実行環境で、`tests/oracle/dispatch-cases.json`が`native-cut-commands`を基礎に生成する明示fixture `native-dispatch-commands`についてcompile manifest、Interpreter/AOT trace、公式差分を
同一fixture・siteで突き合わせられた一意名331 entryと、明示catalog IDで固定したplugin_system側日時27 entryが`trace-confirmed-unattested`です（既存14命令に型変換系16命令、算術・比較系15命令、配列系12命令、文字列系21命令、論理・ビット・集約系20命令、表系14命令、数学系36命令、文字変換系4命令、置換系2命令、出現1命令、形式・文字種判定系7命令、値・状態系12命令、文字列合成・URL系12命令、JSON・要素数系9命令、幅変換系8命令、配列callback系4命令、配列構造系7命令、辞書・hash key系7命令、配列スタック・複製・連番系5命令、パス系5命令、礼節系6命令、動的実行系2命令、標準出力系6命令、デバッグ待機系1命令、デバッグ表示・配列シャッフル系2命令、システム関数存在系1命令、実行時間計測系1命令、ハテナ関数設定・実行系2命令、システムメタデータ一覧系5命令（システム関数一覧取得、プラグイン一覧取得、モジュール一覧取得、助詞一覧取得、予約語一覧取得）、Node HTTP body系1命令（POSTデータ生成）、Node状態・ファイル情報系3命令（AJAXオプション設定、ファイルサイズ取得、ファイル情報取得）、Nodeファイル存在系2命令（存在、フォルダ存在）、Node基本ファイルI/O系3命令（開、読、バイナリ読）、Node文字コードファイルI/O系4命令（SJISファイル読、SJISファイル保存、EUCファイル読、EUCファイル保存）、Nodeファイル操作系5命令（フォルダ作成、保存、ファイル列挙、全ファイル列挙、ファイル削除）、Nodeファイルcopy・move系4命令（ファイルコピー、ファイル上書コピー、ファイル移動、ファイル上書移動）、Node cwd変更系2命令（カレントディレクトリ変更、作業フォルダ変更）、CSV系7命令、秒待系8命令、Promise成功系3命令、Promise処理系2命令、Promise束ね系1命令、Node host情報・環境系10命令、Node一時・乱数系3命令（一時フォルダ作成、ランダムUUID生成、ランダム配列生成）、Node archive state系1命令（圧縮解凍ツールパス変更）、ハッシュ値計算1命令、Node文字コード系4命令、日時書式系1命令、時間・乱数系4命令、グローバル関数一覧取得1命令、Node console・assertion系2命令（コンソールクリア、ASSERT等）を加えた範囲）。さらに`native-scalar-system-constants`の24件は、17件のglobal readと7件のtyped literalを、専用manifest／Interpreter・AOT trace／公式4経路比較で`lnako.static-constant-evidence.v2`へ別namespaceとして記録する。`native-string-system-constants`の24件は公式sourceをoracleにした比較で、`lnako.static-constant-evidence.v2`へ別namespaceとして記録する。公式生成JavaScriptには初期`名前空間`の既知差があるため、string fixtureでは公式source・lnako run・AOT O0の一致と公式生成JavaScriptの成功を記録する。`native-array-system-constants`の2件は、共有配列を変更してから表示する公式source・生成JavaScript・lnako run・AOT O0の一致を同じ証拠形式で記録し、`native-datetime-era-data`の`plugin_system`／`command-0227` global read 1件は、同名`plugin_datetime` entryと区別する明示catalog IDを含む公式source・公式生成JavaScript・`lnako run`・AOT O0の一致を同じ証拠形式で記録する。`native-node-archive-constant`の`plugin_node` global read 1件は公式カタログの初期値`7z`を読む公式source・公式生成JavaScript・`lnako run`・AOT O0の一致を同じ証拠形式で記録する。`native-node-command-line-constants`の`plugin_node` global read 3件は、argv由来の非空条件を公式source oracle・lnako run・AOT O0で比較し、同じ証拠形式で記録する。`native-node-http-initial-constants`の5件は、公式カタログ上の`plugin_node`（`AJAXオプション`）と`plugin_httpserver`（`HTTPメソッド`、`GETデータ`、`POSTデータ`、`FILESデータ`）を同じfixtureでcatalog plugin identity付きで検証する。公式CLI・公式生成JavaScript・`lnako run`・AOT O0の初期値比較では、`AJAXオプション`が空文字、HTTPサーバー未取り込みの4定数が`null`となる公式境界を保持する。`native-node-mother-path`の定数1件は、同じfixtureに含まれる関数呼出しを除外し、`母艦パス`のglobal readだけを公式4経路とInterpreter/AOT trace・manifestで確認する。`native-caniuse-agents`の`plugin_caniuse` global read 1件は、19キーの`ブラウザ名変換表`を公式source・`lnako run`・AOT O0で一致させる。公式生成JavaScriptはcaniuse pluginをstandaloneへ登録しないため生成自体は成功してもstdoutが異なり、この既知差は`docs/COMPATIBILITY_QUIRKS.md`で記録する。`native-system-promise-reject`の`plugin_promise` global read 1件は、Promise生成後に更新された`そ`を公式4経路とInterpreter/AOT trace・manifestで確認する。`native-node-file-copy-default`の`ファイルコピーデフォルト動作`は、可変globalの初期値と`上書`・`overwrite`代入をread/write/read/write/readの5 siteで記録し、公式source・公式生成JavaScript・`lnako run`・AOT O0の結果とcatalog IDを`lnako.global-binding-evidence.v1`へ固定する。追跡中のJSONにはattestationを記録しないため、ローカルの`verified`は0件で、合計445 entryが`trace-confirmed-unattested`、82 entryが`unverified`です。
...
# 終了コード・SIGINT callbackを通常dispatch証拠から分離して検証（Windowsでは実コンソールイベント未対応のため実行不可）
node tools/check_node_exit_evidence.mjs --no-build --output /absolute/path/expected-exit-evidence.json
...
- `compat/v3.7.24/static-datetime-plugin-era-constant-evidence.json`（`native-datetime-plugin-era-data`の`plugin_datetime`／`command-0807` global read 1件の実行証拠）
...
- `compat/v3.7.24/global-binding-evidence.json`（`native-node-file-copy-default`の可変global read/write 5件の実行証拠）
- `compat/v3.7.24/directory-binding-evidence.json`（`native-node-directory-values`の`デスクトップ`・`マイドキュメント`・`テンポラリフォルダ` global read 3件の実行証拠）
...
`plugin-httpserver-all`は、既存のInterpreter比較に加えて、純LLVM AOT O0〜O3の実通信比較を行うfixtureです。HTTPクライアントは`tools/compare_node_http_oracle.mjs`でloopback実通信を比較し、`plugin-node-http-callbacks`、`plugin-node-http-options-and-promises`、`plugin-node-http-async-values`、Discord、失敗処理、LINE Notify廃止命令の捕捉を含む9 fixtureをAOT O0〜O3へ昇格しています。LINE Notifyは直接実行の終了状態差を保持するfixtureと、`エラー監視`で命令固有の廃止本文を捕捉して失敗dispatchを記録するfixtureを分けています。
...
`trace-confirmed-unattested` 527、`unverified` 0です。
...
Unicode propertyの補集合は、ECMAScriptの[`CompileToCharSet`](https://tc39.es/ecma262/2024/multipage/text-processing.html#sec-runtime-semantics-compiletocharset)に合わせ、`u`と`v`でcase foldingと補集合の順序を分けます。`native-system-regexp-unicode-ignore-case`では`\\P{Lowercase_Letter}`の`iu`／`iv`差と、`ASCII`の`iv`正負集合を公式CLI・生成JavaScript・Interpreter・LLVM AOTで比較します。これは通常のregex runtimeへJavaScriptを混入させない共有Zig実装の実行証拠であり、文字列property escapeなど未対応境界は`TODO: regexp-unicode-flags`として残します。

`native-system-dictionary-inherited-enumeration`は、custom辞書prototype chainの列挙順・shadowing・対応値を公式CLI・公式生成JavaScript・`lnako run`・LLVM AOT O0〜O3で比較するfixtureです。TOMLの生成standalone差分fixtureを含む現行fixture inventoryは全414件（AOT 312件、Interpreter 112件、QuickJS 9件）です。
...
`LNAKO_GLOBAL_TRACE`はInterpreter/AOTの成功した`global-read`と`global-write`をschema 1で記録する。
...
それぞれ`lnako.static-constant-evidence.v2`として記録しています。可変globalのread/write境界は別に
`node tools/check_global_binding_evidence.mjs --no-build`で実行し、`native-node-file-copy-default`を
`compat/v3.7.24/global-binding-evidence.json`（`lnako.global-binding-evidence.v1`）へ記録します。
同じ検査器を`--profile node-directory`で実行し、`native-node-directory-values`の3 global-read siteを
`compat/v3.7.24/directory-binding-evidence.json`（`lnako.global-binding-evidence.v2`）へcatalog ID別に記録します。OS依存の実値はartifactへ保存せず、公式source・公式生成JavaScript・`lnako run`・AOT O0の結果hashとmanifest／traceだけを固定します。
attestationはまだ付いていないため、
...
`native-system-reference-byte-buffer-properties`は、Buffer/Uint8Arrayの数値添字・`length`・`buffer`と、`buffer`から得たArrayBufferの`byteLength`・欠損添字を`参照`/`配列参照`で読む境界について、公式CLI・生成JavaScript・`lnako run`・LLVM AOT O0〜O3を比較します。fixture inventoryは全414件（AOT 312件、Interpreter 112件、QuickJS 9件）へ更新しています。
