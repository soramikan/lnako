...
> 旧版の保全文書です。現行の説明は [`../ARCHITECTURE.md`](../ARCHITECTURE.md) を参照してください。
...
- `plugins/system` の`ナデシコ`/`ナデシコ続`は、文字列ソースを埋め込みZig Interpreterで再度構文解析・意味解析・SSA検証・実行する`dynamic-execute` routeである。AOTは動的Interpreterのグローバルと`表示ログ`を外側のAOT値へ同期し、JavaScript runtimeを組み込まない。`native-system-dynamic-execution`で公式ソース・`lnako run`・LLVM AOT O0〜O3を比較し、公式生成JavaScript単体が異なる既知の経路差は互換quirkへ記録する。
...
- `plugins/system` の`ハテナ関数設定`は、非JSの関数値またはシステム命令名配列をGCルート付きcallback列へ保持する。`ハテナ関数実行`は設定済みcallbackを左から順に適用し、未設定時は`デバッグ表示`と同じ位置付きJSON出力へ接続する。AOTは`hatena-configure`で設定し、`hatena-default` ABIで設定済み列または既定動作を実行し、`native-system-hatena-custom`と`native-system-hatena-default`で公式7経路差分を検証する。`JS:`コールバックは通常AOTへ持ち込まず、明示的なcompat-js境界に残す。
...
- Node/AJAXの初期定数5 entryは、`native-node-http-initial-constants`で公式CLI・生成JavaScript・Interpreter・LLVM O0〜O3を比較する。`plugin-httpserver-all`はHTTPサーバーの実通信を公式処理系と純LLVM AOT O0〜O3で比較し、AOTはJavaScript runtimeを使わないZigのlistener・HTTP/1.1 parser・query/form/multipart parser・静的配信・callback応答を実行する。Windowsの外部コンソールイベントなど、このfixtureが発生させないホスト境界は別TODOとして残る。
- Nodeの`LINE送信` / `LINE画像送信`は廃止済みAPIを呼び出さず、固定v3.7.24の[`plugin_node.mts`](https://github.com/kujirahand/nadesiko3/blob/aa18c7e640523938c680958fe731418cc6f7a58f/src/plugin_node.mts#L1604-L1617)にある命令固有の廃止エラー本文へ接続する。InterpreterとAOTは同じ本文を`エラーメッセージ`へ設定する。公式CLIはstdoutへ表示して終了コード0、公式生成JavaScriptは実行時エラーで終了コード1になる既知の経路差があるため、直接実行fixtureは公式生成JavaScriptをoracleにし、`*-captured` fixtureで失敗dispatchと捕捉後の本文を7経路比較する。QuickJS互換モードはこの標準命令の証拠経路に含めない。
- Nodeの`強制終了時`は、登録したcallbackへ未定義値を渡し、callbackがtruthyを返したときだけ終了コード0で終了し、falseyなら処理を継続する。InterpreterはNode hostの割り込みpoll、純LLVM AOTはJS runtimeを使わないPOSIX SIGINT／Windowsコンソール制御イベントhandlerと20ms待機sliceのpollへ接続する。`native-node-interrupt-registration`で7経路の登録・false継続を比較し、`compare_node_interrupt_oracle.mjs`ではPOSIXの公式／Interpreter／AOT O0〜O3 SIGINT差分を比較する。Windows外部コンソール制御イベントをこのハーネスから発生させる回帰は未実装境界として残る。
...
- Nodeの`尋` / `文字尋` / `標準入力全取得` / `標準入力取得時`は、AOTランタイムが標準入力を一度だけ最大64 MiBでバッファする。行命令とcallbackでは行末直前のCRだけを除去して行を渡し、EOFで末尾改行のない部分行も返す。`尋`だけNodeの`Number`相当へ変換し、全取得はrawのUTF-8文字列を返す。`標準入力取得時`は各行をシステムグローバル`対象`へ設定して登録コールバックへ同期的に渡す。公式の[`plugin_node`命令一覧](https://nadesi.com/v3/doc/index.php?plugin_node=&show=)と固定upstreamの[`plugin_node.mts`](https://github.com/kujirahand/nadesiko3/blob/aa18c7e640523938c680958fe731418cc6f7a58f/src/plugin_node.mts)を参照し、`native-node-stdin-lines`、`native-node-stdin-callback`、`native-node-stdin-all`でプロンプト、CRLF、raw全入力、EOF部分行、全行callbackとの7経路差分を検証する。
- Nodeの`POSTデータ生成`は、辞書の挿入順を保ったままキーと値をUTF-8へ変換し、URI component形式の`key=value&...`文字列を生成する。純LLVM AOTはNode/JavaScriptへ依存せず同じunreserved文字集合とUTF-8バイト単位のパーセントエンコードを使い、`native-node-http-post-data`で公式CLI・生成JavaScript・Interpreter・LLVM O0〜O3を比較する。さらにNode HTTPクライアント19 entryを純Zigの`std.http.Client`へ接続し、`plugin-node-http-callbacks`、`plugin-node-http-options-and-promises`、`plugin-node-http-async-values`、Discord、失敗処理の公式差分を`tools/compare_node_http_oracle.mjs`で検証する。
- Nodeの`AJAXオプション設定`は、設定値を`AJAXオプション`グローバルへ保持し、HTTPクライアントの共通リクエストへ適用する。純LLVM AOTは専用ABIでGC管理グローバルへ値を直接保存し、callback・Promise・同期値のHTTP routeがその値を読み取る。`native-node-http-options-set`と`plugin-node-http-options-and-promises`で公式CLI・生成JavaScript・Interpreter・LLVM O0〜O3を比較する。
- Nodeの`AJAX失敗時`は、失敗時コールバック値を`AJAX:ONERROR`グローバルへ保持し、HTTPクライアントのネットワーク失敗時にその関数値をイベントドレインから呼び出す。純LLVM AOTは専用ABIで関数値をGC管理グローバルへ保存し、通常のHTTP失敗はPromiseを拒否する。`plugin-node-http-callbacks`、`plugin-node-http-discord-failure`、`plugin-node-http-onerror`で公式CLI・生成JavaScript・Interpreter・LLVM O0〜O3を比較する。
- Nodeの`圧縮解凍ツールパス`は`7z`を初期値とする可変システムグローバルで、`圧縮解凍ツールパス変更`はその値を更新する。純LLVM AOTは共有システム定数と専用`archive-tool-path` ABIでこの状態遷移を保持する。`圧縮`・`解凍`は未変更時に共有pure-Zig stored-ZIP実装を同期実行し、明示変更後は指定ツールへ公式と同じ`a -r` / `x -o... -y` argvで委譲する。`圧縮時`・`解凍時`はsource/destination/tool pathをGC外のタスクへコピーし、AOTイベントドレイン時に結果をcallbackへ渡す。`plugin-node-native-archive`で通常経路を、`plugin-node-native-archive-hermetic`で公式source・生成JavaScript・Interpreter・AOT O0〜O3のhelper差分とZIP意味結果を検証する。後者のhelperはテスト時の明示markerに限り有効で、通常の外部7z経路を置き換えず、任意の外部7z実装や3 OS attestationまでは証明しない。
