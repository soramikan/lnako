...
- lnakoの現在動作: semantic analyzer が関数スコープへ `引数` をローカル変数として宣言し、HIR lowererが関数先頭で `arguments_array` 命令を発行して束縛します。`arguments_array` は呼出し時の実引数列をそのまま配列化するため、関数値呼び出しで仮引数より多く渡した実引数も要素に残ります。`引数[i]` は公式と同じ実引数を返し、束縛は呼出しごとのフレームに閉じるため入れ子呼出しでも壊れません。
- 判定: 仕様（実引数の並びと呼出しごとの束縛・余剰実引数の保持）／意図的制限（`引数の要素数` は公式より1小さい。公式の末尾要素はJS実装の実行コンテキスト `__self` であり、lnakoには対応する値がないため合成しません）
- 実測した周辺仕様: 公式は関数内の `引数` を「既定で定義済みの変数名」として未定義警告の対象外にしつつ（`nako_gen.mts` の `#952` 例外。`!厳チェック`でも `引数` 自体は警告されません）、名前を参照した時点で変数リストへ登録します。そのため `定数 引数=7` だけの関数は受理され、`引数=8` や `引数[0]を表示` のように先に `引数` を使ったあとの `定数 引数=7` は「定数『引数』の二重定義はできません。」で拒否されます。lnakoは代入（`引数=8`）と増減（`引数を1増やす`）を公式と同じ「定義」として扱い、後続の明示宣言を `duplicate_symbol`（「『引数』は同じスコープで既に定義されています」）として同じ宣言行で報告します（同名通常変数の `A=8;定数 A=7` と同じ検出段階で、文言のみ公式と異なります）。宣言より前の代入は定数代入として拒否しません（公式も拒否しません）。なお、読み出しだけの先行使用（`引数[0]を表示`）や添字代入（`引数[0]=8`）を変数の定義とみなして後続の明示宣言を拒否する差は通常変数（`A[0]=8;定数 A=7`）にも共通する既存差のため、本項の対象外です。プロパティ代入（`引数$a=1`）は登録済みの名前だけを対象にするため、その文の位置までに `引数` が未登録なら「変数『引数』が見当たりません。」になります。lnakoは最初の登録位置を結合ストリーム上の順位で保持し、使用文の順位と比較して判定します（関数内取り込みの展開位置も同じ順序に含まれます）。
- 対象経路: Interpreter / AOT
- 差分テストID: `arguments-array-in-function`、`arguments-array-per-call`、`arguments-array-local-declaration`、`arguments-array-indexed-write-only`、`arguments-array-function-import`、`arguments-array-call-value-extra`、`arguments-array-call-value-less`、`arguments-array-call-value-exact`、`arguments-array-call-value-nested`、`arguments-array-property-after-registration`（`compare_interpreter_oracle.mjs`）、`native-arguments-array-in-function`、`native-arguments-array-indexed-write-only`、`native-arguments-array-per-call`、`native-arguments-array-anonymous`、`native-arguments-array-local-declaration`、`native-arguments-array-call-value-extra`、`native-arguments-array-call-value-less`、`native-arguments-array-call-value-exact`、`native-arguments-array-call-value-nested`、`native-arguments-array-property-after-registration`（`compare_native_oracle.mjs`）、`関数内の『引数』は実引数の配列になる`、`『引数』は呼出しごとに独立し入れ子呼出しで壊れない`、`『引数』宣言は同名ローカルとして再利用する`、`仮引数名が『引数』でも実引数配列を参照する`、`関数値呼び出しの『引数』は余剰実引数を保持する`、`関数値呼び出しの不足実引数は『引数』へ含めず仮引数だけ埋める`、`読み出しの無い『引数』添字代入でも先頭束縛を作る`（`src/runtime/interpreter/tests.zig`）、`AOT関数呼出しの個数契約は埋め込みcallbackと生成wrapperで分かれる`（`src/runtime/aot/tests.zig`）、`semantic-diagnostic-arguments-duplicate-declaration`、`semantic-diagnostic-arguments-const-after-assignment`、`semantic-diagnostic-arguments-property-write-unregistered`、`semantic-diagnostic-arguments-property-write-before-read`、`semantic-diagnostic-arguments-property-write-before-assign`、`semantic-diagnostic-arguments-property-write-before-decl`、`semantic-diagnostic-arguments-property-write-inside-if`（`compare_semantic_diagnostics_oracle.mjs`）、`仮引数『引数』と同名宣言の重複診断を保つ`、`『引数』の明示宣言は同名ローカルとして再利用し宣言前の代入を拒否しない`、`未登録の『引数』へのプロパティ代入は未定義として診断する`、`厳チェックでも関数内の『引数』は未定義にならない`（`src/semantic/analyzer_arguments_test.zig`）
