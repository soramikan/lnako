...
- 判定: 仕様
- 対象経路: Interpreter / AOT
- 差分テストID: `取り込み文の位置で取り込み先のトップレベルを実行する`、`取り込みはネストしても取り込み文位置の実行順を保つ`、`同一モジュールの重複取り込みは最後の取り込み文位置で一度だけ実行する`、`循環取り込みはエントリ内容を一度だけ再展開する`、`関数本体内の取り込みは呼び出し毎に実行する`、`関数本体内の取り込みは入れ子の取り込みも呼び出し毎に実行する`、`ループ本体内の取り込みは繰り返し毎に実行する`、`エントリの自己取り込みは一度だけ再展開する`（`src/runtime/interpreter/tests.zig`）
- 既知の近似差: 関数本体内取り込みの**変数スコープ**。公式は取り込み先のトップレベル変数を取り込み文位置のスコープ（関数ローカル）へ束縛するが、lnakoはモジュール変数をグローバルへ束縛するため関数外から参照できる（main由来の既存差異、#74）。
- TODO識別子: `TODO: import-function-scope-leak`

### 循環取り込み再展開の構文モード（既知の近似差）

- 公式実測・source根拠: 循環取り込みで再展開されるエントリのコピーは、その循環取り込み文の位置で有効だった構文モードを初期モードとして展開されます（コピー内の取り込み文はinclude guardで除去されるためtailモードも載りません）。エントリ本体とは異なるモードのコピーが実行され得ます。
- lnakoの現在動作: 再展開コピーは対象モジュールのコンパイル済み本体を共有するため、コピー専用の解析モードを表現できません。共有本体がコピーを忠実に表現できるのは「循環位置のモード = 対象の解析開始モード（初期＋強制）」かつ「対象へtail適用されたモードが全てその位置の文由来モードに含まれる」場合だけであり、伝搬確定後に `checkCircularCopyModes` がこれを検査して条件を満たさない場合は `invalid_import` 診断でコンパイルを中止します（誤った添字規則で実行するより明示的な失敗を優先）。終端モードが一致しても解析開始モードが異なる場合（例: エントリが標準モードで開始し `DNCLモード` 文で有効化した後に取り込む）は診断対象です。文脈別エントリ生成による完全対応は未実装です。
- 判定: 部分互換（不一致経路は診断化）
- 対象経路: Interpreter / AOT
