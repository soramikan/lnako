...
- lnakoの現在動作: 再展開コピーはエントリのコンパイル済みエントリ関数を共有するため、コピー専用の位置モードを表現できません。伝搬確定後に `checkCircularCopyModes` が「循環取り込み位置のモード ≠ エントリ終端モード」を検出した場合は `invalid_import` 診断でコンパイルを中止します（誤った添字規則で実行するより明示的な失敗を優先）。コピー側にだけ現れるモードbitの位置差は検出できない残余の近似差があります。文脈別エントリ生成による完全対応は未実装です。
- 判定: 部分互換（不一致経路は診断化）
- 対象経路: Interpreter / AOT
