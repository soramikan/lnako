...
- lnakoの現在動作: 上限を設けて位置付き診断`nesting_too_deep`（『式や命令の入れ子が深すぎます』）へ収束させます。再帰下降の入れ子は`max_parse_nesting_depth`（1,024）で数え、パーサ自身が深い括弧・ブロックでプロセススタックを使い切るのを防ぎます。連鎖呼出しと左入れ子の演算子はパーサの再帰を深くしないまま深いASTを作るため、解析後に明示的なスタックでASTの深さを測り`max_ast_depth`（2,048）で判定します。両上限とも公式が受理する段数（連鎖2,000段・`+`2,000項・括弧1,000段）を含み、公式が失敗する段数（3,000段・2,000段）を下回ります。公式の失敗境界はJSのスタック残量に依存するため、実測していない帯（連鎖2,049〜2,999段など）では公式より厳しくなる可能性があります。関数内取り込みの展開子（`ast.Node.expansion`）はファイル単体の解析時検査より後に接続されるため、接続後に`children`と`expansion`を合わせた深さを測り直します（超過時は、展開子の位置が別モジュールの複製なので、その展開を導入した取り込み文の位置を診断します）。
