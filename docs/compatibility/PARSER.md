...
- lnakoの現在動作: パーサが助詞付きの既知命令名を連鎖呼出しとして解決し、公式と同じ入れ子の`func`ノードを作ります（`それは「abc」`に続く`大文字変換を表示。`は`ABC`を出力）。省略引数の「それ」補完は従来どおり中間表現loweringで行うため、ASTには`それ`を挿入しません。
- 判定: 仕様（命令名一覧に無い名前とユーザー定義関数を連鎖の先頭に置く形は未対応）
- 対象経路: Parser / Interpreter / AOT
