...
現行HEADでは、明示`commands`を追加した既存AOT fixtureと数学・日時・caniuse・漢数字・CSV・TOML・マークアップ・比較・表・敬語 fixtureを含めて、fixture inventoryは
全242件（AOT 137件、Interpreter 96件、QuickJS 9件）です。命令のfixture coverageは`paired` 363 entry、
`interpreter-only` 160 entryです。execution evidenceは`verified` 0、
