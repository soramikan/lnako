...
別ファイルの`compat/v3.7.24/dispatch-coverage-evidence.json`は、canonical dispatch証拠とは分離したサンプル監査です。32 fixture・1,698 siteの成功経路と明示した期待失敗経路から、一意名として同定できる311 native entryを記録し、そのうち従来`unverified`だった19 entry（既存17 entryに`__DEBUG`と`エラー発生`を加えたもの）をcatalog evidenceへ接続します。これは公式source・`lnako run`・AOT O0の一致、Interpreter/AOT trace、compile manifestを同一siteで確認した`trace-confirmed-unattested`であり、公式生成JavaScriptのstandalone plugin登録差、3 OS attestation、全527 entryのAOT実行を証明しません。詳細な公式実装の出典と、説明不足・不具合候補・未実装境界の区別は`docs/COMPATIBILITY_QUIRKS.md`へ記録します。

現時点で残る`unverified` 87 entry（初期89 entry）の精査結果、実装単位、証拠基盤の依存関係、各単位の完了条件は[`docs/UNVERIFIED_EVIDENCE_PLAN.md`](UNVERIFIED_EVIDENCE_PLAN.md)を実行計画の正本とする。ここで示す件数は、命令の実装済み件数ではなく、現行HEADでcatalog ID付きの実行証拠へ接続できていない件数である。U01の2 entryは、期待失敗を含むdispatch evidenceを追加して`trace-confirmed-unattested`へ接続済みである。
...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 440`、`unverified: 87`を記録する。catalog再生成時の`--historical-commit`は
...
- `compat/v3.7.24/dispatch-coverage-evidence.json`（32 fixture・1,698 siteのサンプルdispatch監査。catalogへ接続するのは19 entry）
...
`trace-confirmed-unattested` 440、`unverified` 87です。
...
既定の監査はCI時間を抑えるため32 fixtureを対象にします（Nodeプロセス／ファイルcallbackのAOT成功fixtureと、`エラー発生`の明示した期待失敗fixtureを含みます）。標準命令の実行siteを広げて監査する場合は、
