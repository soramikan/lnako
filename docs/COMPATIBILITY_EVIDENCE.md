...
emitter routeは`builtin`・`cut`・`regexp`・`timer`・`promise`・`debug-display`・`hatena-default`・`archive-tool-path`・`ajax-options`・`ajax-onerror`・`node-file-io`・`node-stdin-lines`・`direct-display`のABI分類であり、
