...
`native-system-reference-byte-buffer-properties`は、Buffer/Uint8Arrayの数値添字・`length`・`buffer`と、`buffer`から得たArrayBufferの`byteLength`・欠損添字を`参照`/`配列参照`で読む境界について、公式CLI・生成JavaScript・`lnako run`・LLVM AOT O0〜O3を比較します。fixture inventoryは全391件（AOT 297件、Interpreter 99件、QuickJS 9件）へ更新しています。
