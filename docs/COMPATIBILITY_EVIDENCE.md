...
全297件（AOT 194件、Interpreter 96件、QuickJS 9件）です。`plugin-node-native-archive`は圧縮・解凍4命令を含み、InterpreterとAOT O0〜O3で実行・生成物を検証します。命令のfixture coverageは`paired` 490 entry、
`interpreter-only` 33 entryです。execution evidenceは`verified` 0、
