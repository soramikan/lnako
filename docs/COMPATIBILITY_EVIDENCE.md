...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 442`、`unverified: 85`を記録する。catalog再生成時の`--historical-commit`は
...
- `compat/v3.7.24/dispatch-coverage-evidence.json`（33 fixture・1,703 siteのサンプルdispatch監査。catalogへ接続するのは21 entry）
...
`trace-confirmed-unattested` 442、`unverified` 85です。
...
既定の監査はCI時間を抑えるため33 fixtureを対象にします（Nodeプロセス／ファイルcallbackのAOT成功fixture、`エラー発生`の明示した期待失敗fixture、`ナデシコ`／`ナデシコ続`のdynamic-execute fixtureを含みます）。標準命令の実行siteを広げて監査する場合は、
