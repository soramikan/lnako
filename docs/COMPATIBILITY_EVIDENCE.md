...
現行HEADでは、明示`commands`を追加した既存AOT fixtureと数学・日時・caniuse・漢数字・CSV fixtureを含めて、fixture inventoryは
全237件（AOT 132件、Interpreter 96件、QuickJS 9件）です。命令のfixture coverageは`paired` 349 entry、
`interpreter-only` 174 entryです。execution evidenceは`verified` 0、
