...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 459`、`unverified: 68`を記録する。catalog再生成時の`--historical-commit`は
...
- `compat/v3.7.24/dispatch-coverage-evidence.json`（38 fixture・1,737 siteのサンプルdispatch監査。catalogへ接続する326 native entryを含む。U06のarchive helperとU08のstdin fixtureを含む）
...
`trace-confirmed-unattested` 459、`unverified` 68です。
...
既定の監査はCI時間を抑えるため38 fixtureを対象にします（Nodeプロセス／ファイルcallbackのAOT成功fixture、`エラー発生`の明示した期待失敗fixture、`ナデシコ`／`ナデシコ続`のdynamic-execute fixture、`ブラウザ起動`／`エクスプローラー起動`の安全なlauncher fixture、`plugin-node-native-archive-hermetic`のstored-ZIP helper fixture、`native-node-stdin-lines`／`native-node-stdin-callback`／`native-node-stdin-all`のstdin fixtureを含みます）。標準命令の実行siteを広げて監査する場合は、
