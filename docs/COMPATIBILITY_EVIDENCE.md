...
現行HEADでは、明示`commands`を追加した既存AOT fixtureと数学・日時・caniuse・漢数字・CSV・TOML・マークアップ fixtureを含めて、fixture inventoryは
全239件（AOT 134件、Interpreter 96件、QuickJS 9件）です。命令のfixture coverageは`paired` 353 entry、
`interpreter-only` 170 entryです。execution evidenceは`verified` 0、
