...
現行HEADでは、明示`commands`を追加した既存AOT fixtureと数学・日時・caniuse・漢数字 fixtureを含めて、fixture inventoryは
全236件（AOT 131件、Interpreter 96件、QuickJS 9件）です。命令のfixture coverageは`paired` 342 entry、
`interpreter-only` 181 entryです。execution evidenceは`verified` 0、
