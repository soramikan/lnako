...
現行HEADでは、明示`commands`を追加した既存AOT fixtureと数学・日時・caniuse・漢数字・CSV・TOML fixtureを含めて、fixture inventoryは
全238件（AOT 133件、Interpreter 96件、QuickJS 9件）です。命令のfixture coverageは`paired` 351 entry、
`interpreter-only` 172 entryです。execution evidenceは`verified` 0、
