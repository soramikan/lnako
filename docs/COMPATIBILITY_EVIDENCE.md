...
現行HEADでは、明示`commands`を追加した既存AOT fixtureと数学・日時・caniuse・漢数字・CSV・TOML・マークアップ・比較 fixtureを含めて、fixture inventoryは
全240件（AOT 135件、Interpreter 96件、QuickJS 9件）です。命令のfixture coverageは`paired` 355 entry、
`interpreter-only` 168 entryです。execution evidenceは`verified` 0、
