...
現行HEADでは`verified: 0`、`trace-confirmed-unattested: 452`、`unverified: 75`です。U05の`native-node-directory-values`に加え、U06の`plugin-node-native-archive-hermetic`を反映しています。前者はOS依存値を固定文字列にせず、`デスクトップ`・`マイドキュメント`・`テンポラリフォルダ`のglobal-read 3 siteを`directory-binding-evidence.json`へcatalog ID別に記録します。
...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 452`、`unverified: 75`を記録する。catalog再生成時の`--historical-commit`は
...
`trace-confirmed-unattested` 452、`unverified` 75です。
