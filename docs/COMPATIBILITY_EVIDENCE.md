...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 461`、`unverified: 66`を記録する。catalog再生成時の`--historical-commit`は
...
- `compat/v3.7.24/dispatch-coverage-evidence.json`（39 fixture・1,743 siteのサンプルdispatch監査。catalogへ接続する328 native entryを含む。U06のarchive helper、U08のstdin fixture、U09のnetwork topology fixtureを含む）
...
`trace-confirmed-unattested` 461、`unverified` 66です。
...
`native-system-dictionary-inherited-enumeration`は、custom辞書prototype chainの列挙順・shadowing・対応値を公式CLI・公式生成JavaScript・`lnako run`・LLVM AOT O0〜O3で比較するfixtureです。TOMLの生成standalone差分fixtureを含む現行fixture inventoryは全398件（AOT 303件、Interpreter 99件、QuickJS 9件）です。
...
既定の監査はCI時間を抑えるため39 fixtureを対象にします（Nodeプロセス／ファイルcallbackのAOT成功fixture、`エラー発生`の明示した期待失敗fixture、`ナデシコ`／`ナデシコ続`のdynamic-execute fixture、`ブラウザ起動`／`エクスプローラー起動`の安全なlauncher fixture、`plugin-node-native-archive-hermetic`のstored-ZIP helper fixture、`native-node-stdin-lines`／`native-node-stdin-callback`／`native-node-stdin-all`のstdin fixtureと`native-node-network-addresses`のsynthetic-v1 network fixtureを含みます）。標準命令の実行siteを広げて監査する場合は、
...
`native-system-reference-byte-buffer-properties`は、Buffer/Uint8Arrayの数値添字・`length`・`buffer`と、`buffer`から得たArrayBufferの`byteLength`・欠損添字を`参照`/`配列参照`で読む境界について、公式CLI・生成JavaScript・`lnako run`・LLVM AOT O0〜O3を比較します。fixture inventoryは全398件（AOT 303件、Interpreter 99件、QuickJS 9件）へ更新しています。
