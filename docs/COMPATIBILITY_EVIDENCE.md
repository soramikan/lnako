...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 379`を記録する。catalog再生成時の`--historical-commit`は
...
`trace-confirmed-unattested` 379、`unverified` 148です。
...
`native-scalar-system-constants`と`native-string-system-constants`について、公式source、公式生成JavaScript、`lnako run`、
AOT O0のstdout/stderr、global/literal manifest、Interpreter/AOT traceを突き合わせます。前者のglobal read 17件＋typed literal 7件と、後者のglobal read 24件をcatalog IDへ一意に解決し、
`compat/v3.7.24/static-constant-evidence.json`と`compat/v3.7.24/static-string-constant-evidence.json`へ
それぞれ`lnako.static-constant-evidence.v2`として記録しています。attestationはまだ付いていないため、
この48件は`trace-confirmed-unattested`であり、`verified`ではありません。
