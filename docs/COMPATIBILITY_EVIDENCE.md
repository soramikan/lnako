...
残る`unverified` 89 entryの精査結果、実装単位、証拠基盤の依存関係、各単位の完了条件は[`docs/UNVERIFIED_EVIDENCE_PLAN.md`](UNVERIFIED_EVIDENCE_PLAN.md)を実行計画の正本とする。ここで示す件数は、命令の実装済み件数ではなく、現行HEADでcatalog ID付きの実行証拠へ接続できていない件数である。
...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 438`、`unverified: 89`を記録する。catalog再生成時の`--historical-commit`は
...
`trace-confirmed-unattested` 438、`unverified` 89です。
