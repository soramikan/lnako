...
`native-node-path-components`はdrive-relative path、drive root、UNC rootを含むWindows形式のbasename/dirname入力を含み、公式CLI・生成JavaScript・Interpreter・LLVM O0〜O3で比較します。Windowsのnamespace path、特殊なUNC形式、入力型エラーの残境界は`TODO: node-path-win32-boundary`として分離します。
