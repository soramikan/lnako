...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 467`、`unverified: 60`を記録する。catalog再生成時の`--historical-commit`は
...
- `compat/v3.7.24/dispatch-coverage-evidence.json`（43 fixture・1,820 siteのサンプルdispatch監査。catalogへ接続する348 native entryを含む。U06のarchive helper、U08のstdin fixture、U09のnetwork topology fixture、U10/U11/U12のloopback HTTP fixtureを含む）
...
`trace-confirmed-unattested` 479、`unverified` 48です。
