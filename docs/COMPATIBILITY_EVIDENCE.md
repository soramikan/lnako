...
現行HEADでは`verified: 0`、`trace-confirmed-unattested: 448`、`unverified: 79`です。U05の`native-node-directory-values`について、OS依存値を固定文字列にせず、`デスクトップ`・`マイドキュメント`・`テンポラリフォルダ`のglobal-read 3 siteを`directory-binding-evidence.json`へcatalog ID別に記録しています。
別ファイルの`compat/v3.7.24/dispatch-coverage-evidence.json`は、canonical dispatch証拠とは分離したサンプル監査です。34 fixture・1,707 siteの成功経路と明示した期待失敗経路から、一意名として同定できる315 native entryを記録し、そのうち従来`unverified`だった23 entry（既存17 entryに`__DEBUG`、`エラー発生`、`ナデシコ`、`ナデシコ続`、`ブラウザ起動`、`エクスプローラー起動`を加えたもの）をcatalog evidenceへ接続します。これは公式source・`lnako run`・AOT O0の一致、Interpreter/AOT trace、compile manifestを同一siteで確認した`trace-confirmed-unattested`であり、公式生成JavaScriptのstandalone plugin登録差、3 OS attestation、全527 entryのAOT実行を証明しません。詳細な公式実装の出典と、説明不足・不具合候補・未実装境界の区別は`docs/COMPATIBILITY_QUIRKS.md`へ記録します。

現時点で残る`unverified` 79 entry（初期89 entry）の精査結果、実装単位、証拠基盤の依存関係、各単位の完了条件は[`docs/UNVERIFIED_EVIDENCE_PLAN.md`](UNVERIFIED_EVIDENCE_PLAN.md)を実行計画の正本とする。ここで示す件数は、命令の実装済み件数ではなく、現行HEADでcatalog ID付きの実行証拠へ接続できていない件数である。U01の2 entryは期待失敗を含むdispatch evidence、U02の2 entryはdynamic-executeのdispatch evidence、U03の2 entryはOS別launcher経路と明示した期待失敗を含むdispatch evidence、U04の1 entryは可変globalのread/write site、U05の3 entryはOS依存値を固定しないglobal read siteを別artifactへ固定して、いずれも`trace-confirmed-unattested`へ接続済みである。
...
追跡中の`compat/v3.7.24/evidence.json`は`verified: 0`、`trace-confirmed-unattested: 448`、`unverified: 79`を記録する。catalog再生成時の`--historical-commit`は
...
- `compat/v3.7.24/dispatch-coverage-evidence.json`（34 fixture・1,707 siteのサンプルdispatch監査。catalogへ接続するのは23 entry）
...
`trace-confirmed-unattested` 444、`unverified` 83です。
...
既定の監査はCI時間を抑えるため34 fixtureを対象にします（Nodeプロセス／ファイルcallbackのAOT成功fixture、`エラー発生`の明示した期待失敗fixture、`ナデシコ`／`ナデシコ続`のdynamic-execute fixture、`ブラウザ起動`／`エクスプローラー起動`の安全なlauncher fixtureを含みます）。標準命令の実行siteを広げて監査する場合は、
