...
- `runtime/aot_builtin.zig` はAOT対応済み標準命令の正式名・別名を安定した数値IDへ解決する。LLVM生成物は
  ...
  `足`・`合計`・`連続加算`命令の各正式名と別名、および数学38 entry（乱数を含む）、日時12 entry（現在時刻・日付・年月）、caniuseの`対応ブラウザ一覧取得`を実装し、後続命令を個別ABIの増殖なしに追加できる。副作用命令の`二進表示`は同じ変換結果を
...
- `runtime/system_constant.zig` はインタープリタとAOTで共有するシステム定数表を持つ。AOTは実際に参照された
  ...
  `抽出文字列`と`__DEBUGブレイクポイント一覧`も、同じ境界で独立した空配列として生成する。
