...
- Nodeの`尋` / `文字尋` / `標準入力全取得`は、AOTランタイムが標準入力を一度だけ最大64 MiBでバッファする。行命令ではCRを除去して行を返し、`尋`だけNodeの`Number`相当へ変換し、全取得はrawのUTF-8文字列を返す。`native-node-stdin-lines`でプロンプト、CRLF、raw全入力との7経路差分を検証し、`標準入力取得時`の永続callbackは別の未実装境界として残す。
