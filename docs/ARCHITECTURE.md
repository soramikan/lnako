...
- Nodeの`尋` / `文字尋` / `標準入力全取得` / `標準入力取得時`は、AOTランタイムが標準入力を一度だけ最大64 MiBでバッファする。行命令とcallbackでは行末直前のCRだけを除去して行を渡し、`尋`だけNodeの`Number`相当へ変換し、全取得はrawのUTF-8文字列を返す。`標準入力取得時`は各行をシステムグローバル`対象`へ設定して登録コールバックへ同期的に渡す。`native-node-stdin-lines`と`native-node-stdin-callback`でプロンプト、CRLF、raw全入力、全行callbackとの7経路差分を検証する。
