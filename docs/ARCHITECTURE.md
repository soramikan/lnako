...
- `runtime/aot_builtin.zig` はAOT対応済み標準命令の正式名・別名を安定した数値IDへ解決する。LLVM生成物は
  ...
  `文字列変換` / `TOSTR`を実装し、後続命令を個別ABIの増殖なしに追加できる。
- `runtime/system_constant.zig` はインタープリタとAOTで共有するスカラーシステム定数表を持つ。AOTは実際に
  参照された真偽値、数値、`NaN`、無限大、`null`、`undefined`をLLVMグローバルのタグ付き値へ初期化する。
  ヒープ割り当てが必要な文字列・配列システム定数はこの初期化範囲に含めない。
