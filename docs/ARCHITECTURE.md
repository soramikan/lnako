...
- `runtime/aot_builtin.zig` はAOT対応済み標準命令の正式名・別名を安定した数値IDへ解決する。LLVM生成物は
  ...
  `足`・`合計`・`連続加算`命令の各正式名と別名、および数学38 entry（乱数を含む）、日時16 entry（現在時刻・日付・年月・曜日・Unix秒・日時文字列）、URL・Base64 5 entry、Nodeホスト情報8 entry（OS・CPU・環境変数取得・一覧取得・カレントディレクトリ取得2 alias・カレントディレクトリ変更2 alias）、`元号データ`の固定5件、caniuseの`対応ブラウザ一覧取得`と`ブラウザ名変換表`を実装し、後続命令を個別ABIの増殖なしに追加できる。副作用命令の`二進表示`は同じ変換結果を
