...
HTTP命令のdispatch実行証拠は `tests/oracle/http-server-dispatch-cases.json` と
`tools/check_dispatch_coverage.mjs`で別に監査します。このfixtureは各routeへ外部loopback clientから実通信を行い、
公式source・lnako Interpreter・AOT O0のHTTP応答とdispatch traceを同じsiteへ接続します。公式生成JavaScriptの
...
AOTは生成main（Windowsではwmain）から解決済みのplugin pathを登録し、通常モードのJavaScript runtimeを使わず、埋め込みZig
Interpreterをhost callbackのadapterとして利用します。Windowsのwmainはwide argvをUTF-16のままランタイムへ渡します。
