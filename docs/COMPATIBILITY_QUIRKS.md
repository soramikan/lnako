...
| `配列範囲コピー` | 数値指定は要素を返し、要素がオブジェクトならJSON複製する。`先頭`・`末尾`範囲は`slice`した配列全体をJSON複製するため、`undefined`はnullになり、参照とは異なってネストしたコンテナも独立する | AOTは配列範囲の負数・小数・範囲外をJavaScriptの`slice`規則で処理し、範囲結果を深く複製する。範囲の`末尾`がBigIntなど、現行数値変換で扱えない型は`TODO: array-copy-range-coercion`として完成扱いにしない | `plugin-system-array-core`、`native-system-array-copy-reference-commands`、`native-system-array-range-copy-null-error`、`native-system-array-range-copy-receiver-error`、`TODO: array-copy-range-coercion` |
| `参照` / `配列参照` | 文字列・配列の数値指定は要素、範囲指定は`substring`/`slice`の新しいコンテナを返す。文字列範囲だけは始点と終点が逆なら交換する。辞書指定は値をそのまま返し、配列・辞書要素は深く複製しない。不正な文字列範囲は`JSON.stringify`相当の値を`『参照』で文字列型の範囲指定(...)が不正です。`へ埋め込み、undefined・関数は`undefined`、BigInt・循環参照はJSON変換自体の例外を優先する | AOTも文字列のUTF-16コード単位、配列要素の参照、孤立サロゲートを含むエラー文言を保持する。辞書の継承プロパティや、範囲オブジェクトの欠落・異型キーに対するJavaScriptオブジェクト完全互換は`TODO: reference-inherited-properties`として分離する。範囲の`末尾`がBigIntのときの明示的`Number`変換は`TODO: reference-range-bigint-coercion`として完成扱いにしない | `plugin-system-array-core`、`native-system-array-copy-reference-commands`、`native-system-reference-null-error`、`native-system-reference-string-range-error`、`native-system-reference-string-range-error-details`、Zig単体テスト、`TODO: reference-inherited-properties`、`TODO: reference-range-bigint-coercion` |
