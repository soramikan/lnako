...
| `参照` / `配列参照` | 文字列・配列の数値指定は要素、範囲指定は`substring`/`slice`の新しいコンテナを返す。文字列範囲だけは始点と終点が逆なら交換する。辞書指定は値をそのまま返し、配列・辞書要素は深く複製しない | AOTも文字列のUTF-16コード単位と配列要素の参照を保持する。辞書の継承プロパティや、範囲オブジェクトの欠落・異型キーに対するJavaScriptオブジェクト完全互換は`TODO: reference-inherited-properties`として分離する。範囲の`末尾`がBigIntのときの明示的`Number`変換は`TODO: reference-range-bigint-coercion`、文字列の不正範囲エラーに指定値を埋め込む詳細は`TODO: reference-error-message-detail` | `plugin-system-array-core`、`native-system-array-copy-reference-commands`、`native-system-reference-null-error`、`native-system-reference-string-range-error`、`TODO: reference-inherited-properties`、`TODO: reference-range-bigint-coercion`、`TODO: reference-error-message-detail` |
