...
| `環境変数取得` | Nodeの`process.env[KEY]`は環境に存在するキーの値を文字列で返し、未設定キーは`undefined`になる | InterpreterはホストContextの環境表を参照し、純LLVM AOTはJS runtimeを使わずC ABIの環境参照から値を生成する。`環境変数一覧取得`のAOT経路は未実装として扱う | `plugin-node-path-host`、`native-node-environment` |
