...
| クロージャのローカル変数捕捉 | 無名関数作成時の値をコピーするのではなく、外側の可変な束縛を共有する。作成後に外側が更新した値が見え、クロージャ自身の更新も次回呼び出しまで保持される | 非捕捉無名関数はGC管理の関数値と統一ABIでAOT実行する。捕捉ありは可変セルによる束縛共有が未実装で、現行インタプリタは作成時の値コピーのため未互換、AOTは誤った実行ファイルを作らずコンパイル時にソース位置付きで拒否する | `捕捉ありクロージャをソース位置付きで拒否する`、`非捕捉無名関数を統一ABIの関数値へ変換する`、`native-noncapturing-function-value` |
