...
| 同名命令のcatalog IDと実装route | 標準527 entryには同名異pluginが31組あり、命令名だけのfixtureはどのcatalog IDを実行したか証明しない。特に`ファイル名抽出`・`パス抽出`はNode host有効時に`plugin_node`が`plugin_system`より先に選ばれる一方、datetime別名は共通実装へ合流する | catalog identityとimplementation routeを分離する。一意名だけ自動解決し、同名は公式の探索順と実traceで一意に確定できる場合だけIDへ結び付ける。現在はNode側2命令だけを固定し、system側・datetime別名は`unverified`のままにする | `check_dispatch_trace.mjs`、`plugin-node-path-host`、`native-cut-commands` |
| AOT compile manifestの証明範囲 | 最適化前IRにbuiltin callがあること、LLVM最適化後もcallが残ること、実行時に成功することは別の事実である。生成途中で止まったJSONLも完了artifactにはできない | `LNAKO_COMPILE_MANIFEST`はNako最適化前の命令名・canonical opcode・LLVM emitter route・位置だけを記録し、link成功後の`complete` recordがある場合だけ受理する。pluginやcatalog IDはmanifest自身へ持たせずchecker側で限定解決する。失敗時は部分ファイルを削除し、runtime traceと公式差分結果を別途照合する | `check_dispatch_trace.mjs`、`lnako.aot.builtin-manifest.v1` |
