...
| `配列ソート` | `Array.sort()`相当の破壊的な安定ソートで、比較値はToString後のUTF-16コード単位順。明示的`undefined`とholeは比較せず末尾へ移動し、同値の順序は維持する | AOT・インタプリタともbottom-up stable merge sortでO(n log n)を確保し、同一配列を返す。dense表現ではholeと明示的`undefined`を区別できないため`TODO: sparse-array-presence`、AOT関数値の文字列化で関数名を保持しない差は`TODO: aot-function-string-name`を維持する | `native-system-array-sort-commands`、`plugin-system-array-core`、`TODO: sparse-array-presence`、`TODO: aot-function-string-name` |
...
| dispatch evidenceのattestation境界 | JSONのdigestだけではGitHub Actionsの実行者や対象commitを証明できず、未署名の単体ファイルを`verified`へ上げると差し替えを検出できない。署名検証済みという文字列を持つmetadataだけでも偽造できる。fork PRではOIDC／attest権限も与えられない | AOT各正式OSのdispatch JSONを公式`actions/attest`のmulti-subject artifact attestationへ結び付け、専用jobとcatalog同期処理の双方で、生成されたSigstore bundleを公式`gh attestation verify`へ渡す。`--signer-workflow`・`--signer-digest`・`--source-digest`・`--source-ref`・公式OIDC issuer・`--deny-self-hosted-runners`を強制し、bundle SHA-256、3 OSの証拠digest、commit、workflow identityがすべて一致した場合だけ一時catalog evidenceを`verified`として生成する。追跡JSONはnull attestationのままにし、未attest、metadataだけ、権限不足、digest／identity不一致は昇格しない | `tools/verify_dispatch_attestation.mjs`、`tools/sync_compat_evidence.mjs`、GitHub Actions `actions/attest@v4.2.2`（SHA固定）、`gh attestation verify` |
| Windowsのdispatch証拠と改行 | Windowsでは公式cnako3・生成JavaScript・InterpreterがLFを出しても、MSVCリンクしたAOT実行ファイルの標準出力はCRLFになる。生bytes比較では同じ表示結果を不一致にし、Windows証拠だけ生成できない | 既存の全115件AOT差分と同じく、公式比較に使うstdout/stderrはCRLFをLFへ正規化してから比較・SHA-256化する。trace有無の同一実行比較とraw trace／manifest hashは生bytesのまま保持する | `tools/check_dispatch_trace.mjs`、`tools/compare_native_oracle.mjs`、Windows x86_64 `aot` |
...
| AOT関数値の文字列化 | JavaScriptの関数文字列には関数名が含まれ、名前中の数字は`通貨形式`や文字種判定の結果にも影響し得る。現AOT関数オブジェクトはコールバック・引数数・captureだけを保持するため、`function () { [native code] }`へ固定している | `TODO: aot-function-string-name`。関数名をAOTオブジェクトと生成ABIへ保持するまでは、関数値を文字列命令へ渡す型注釈外経路を完成扱いにしない | `TODO: aot-function-string-name` |
