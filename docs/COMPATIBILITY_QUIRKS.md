...
| 同名命令のcatalog IDと実装route | 標準527 entryには同名異pluginが31組あり、命令名だけのfixtureはどのcatalog IDを実行したか証明しない。特に`ファイル名抽出`・`パス抽出`はNode host有効時に`plugin_node`が`plugin_system`より先に選ばれる一方、datetime別名は共通実装へ合流する | catalog identityとimplementation routeを分離する。一意名だけ自動解決し、同名は公式の探索順と実traceで一意に確定できる場合だけIDへ結び付ける。現時点でdispatch証拠を公式差分まで結合して`trace-confirmed-unattested`にできるのは、`native-cut-commands`の一意名4命令（`切取`・`範囲切取`・`CHR`・`表示`）だけであり、外部attestation導入前の`verified`は0件。Node側2命令・system側・datetime別名は`unverified`のままにする | `check_dispatch_trace.mjs`、`dispatch-evidence.json`、`native-cut-commands`、`plugin-node-path-host` |
