...
| 幅変換命令の非文字列引数 | 公式は文字列化せず、カナ全角化ではNumber/真偽値を空、`NULL`/`undefined`やカナ半角化・`半角変換`などではTypeErrorになる。現lnakoの文字列化経路は文字列引数の互換を優先しており、非文字列のこの分岐は未実装 | `TODO: system-width-non-string-coercion` として互換表に残し、完成扱いにしない | `TODO: system-width-non-string-coercion` |
