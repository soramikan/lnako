...
| `参照` / `配列参照` | 文字列・配列の数値指定は要素、範囲指定は`substring`/`slice`の新しいコンテナを返す。文字列範囲だけは始点と終点が逆なら交換する。辞書指定は値をそのまま返し、配列・辞書要素は深く複製しない。不正な文字列範囲は`JSON.stringify`相当の値を`『参照』で文字列型の範囲指定(...)が不正です。`へ埋め込み、undefined・関数は`undefined`、BigInt・循環参照はJSON変換自体の例外を優先する。文字列・配列の範囲末尾は明示的な`Number`変換でBigIntを受理し、配列の単一BigInt添字はプロパティキーとして扱う | AOT・インタープリタは文字列のUTF-16コード単位、配列要素の参照、孤立サロゲートを含むエラー文言を保持し、明示的な範囲終端だけBigInt.toF64相当へ変換する。配列の負数・巨大BigInt単一添字は実用配列長外として`undefined`にする。辞書の継承プロパティは`TODO: reference-inherited-properties`、配列の文字列キー・`length`等のプロパティ互換は`TODO: reference-array-property-key`として分離する | `plugin-system-array-core`、`native-system-array-copy-reference-commands`、`native-system-reference-null-error`、`native-system-reference-string-range-error`、`native-system-reference-string-range-error-details`、`native-system-array-range-copy-bigint-coercion`、Zig単体テスト、`TODO: reference-inherited-properties`、`TODO: reference-array-property-key` |
