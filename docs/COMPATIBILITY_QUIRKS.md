...
| 式中の`表示(...)`の戻り値 | 引数を出力するが、その引数値は返さず`undefined`を返す。`1かつ表示("右")`は右辺の表示後、式全体の値が`undefined`になる | AOTの表示ヘルパーも出力後に`undefined`を返し、呼び出し式の値に引数を流用しない | `native-logical-short-circuit` |
