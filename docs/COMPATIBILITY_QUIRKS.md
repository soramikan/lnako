...
| 配列変更命令の`pure:true`属性 | 公式カタログは`配列挿入`、`配列一括挿入`、`配列削除`、`配列切取`、`配列取出`、`配列ポップ`、`配列プッシュ`、`配列追加`を`pure:true`として登録しているが、実装は元配列・辞書を破壊的に変更する | AOTは元コンテナへの書き込みを保持し、これらの命令を純粋式としてDCE・共通部分式削除する最適化根拠には使わない。カタログ属性と実行時副作用の統一は`TODO: builtin-pure-side-effect-metadata`として分離する | `plugin-system-array-mutation-quirks`、`native-system-array-mutation-commands`、`TODO: builtin-pure-side-effect-metadata` |
