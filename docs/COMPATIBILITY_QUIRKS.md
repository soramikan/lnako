...
| クロージャのローカル変数捕捉 | 無名関数作成時の値をコピーするのではなく、外側の可変な束縛を共有する。作成後に外側が更新した値が見え、クロージャ自身の更新も次回呼び出しまで保持される | インタプリタはGC管理の可変セルを外側フレームとクロージャで共有する。セルから参照する値もGCで追跡する。AOTは非捕捉無名関数だけをGC管理の関数値と統一ABIで実行でき、捕捉ありは誤った実行ファイルを作らずコンパイル時にソース位置付きで拒否する | `クロージャが外側の可変束縛を共有する`、`closure-shared-mutable-binding`、`捕捉ありクロージャをソース位置付きで拒否する`、`native-noncapturing-function-value` |
