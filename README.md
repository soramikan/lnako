...
Nodeの`尋`と`文字尋`は、純LLVM AOTでも標準入力を一度バッファして行単位に分割し、前者だけ数値変換し、後者は文字列のまま返します。`native-node-stdin-lines`でCRLF、プロンプト、後続の`標準入力全取得`を公式CLI・生成JavaScript・Interpreter・LLVM O0〜O3と比較します。
