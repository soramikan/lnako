...
生成コード用ランタイムABIへ接続します。CSV 7 entry（CSV/TSV解析、引用、数値自動変換、セル文字列化、オプション設定）もこのAOT routeに含まれます。未対応IRを含む入力は誤変換せず、命令名と元ソース位置付きで拒否します。この列挙は現行AOT routeの範囲であり、標準527 entry全件のAOT実行証拠を意味しません。
