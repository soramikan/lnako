...
代表値は次の通りです。2026年9月5日、Linux x86_64（AMD EPYC 7763、論理4 CPU）のCIで、同じソースをwarmup 3回・測定10回、AOT O2で比較した**中央値（ms、小さいほど短時間）**です。測定コミットは `9fdd244` です。

| ケース | cnako 3.7.24 | lnako interpreter | lnako AOT O2 |
| --- | ---: | ---: | ---: |
| 空プログラムの起動 | 100.92 | 1.89 | 1.71 |
| 整数演算 | 129.68 | 850.96 | 59.27 |
| 配列構築 | 124.46 | 258.47 | 13.58 |
| 辞書検索 | 145.61 | 573.77 | 4,512.44 |
| JSON変換 | 127.89 | 78.56 | 8.52 |

実行時間は起動・終了を含み、AOTの事前コンパイル時間は含みません。200ms未満の値は起動やCI環境の影響を受けやすく、辞書検索のようにAOTが遅いケースもあります。[全20ケース・3環境の詳細結果](docs/benchmarks/RESULTS.md)と[実行方法](benchmarks/README.md)を参照してください。
