...
現段階のAOT対応は、数値・真偽値・null・NULを含むUTF-16文字列、変数、数値演算・比較、条件・while・後判定・条件分岐、直接関数呼び出し、
表示、文字列・配列・辞書の添字参照、配列・辞書の更新・分割宣言、回数・範囲・型が確定した文字列・配列・辞書の反復です。BigInt、クロージャ、例外、非同期、残りの標準命令は後続の固定マイルストーンで
