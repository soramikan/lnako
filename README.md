...
`verified` 0件、`trace-confirmed-unattested` 8件、`unverified` 519件です。分類やfixtureの存在は、全entryのAOT実行証明を意味しません。
...
現段階のAOT対応は、数値・真偽値・null・NULを含むUTF-16文字列・配列と辞書を含む`&`文字列連結、辞書のカスタム`toString` / `valueOf`を使う文字列・数値hintのToPrimitive・BigInt定数・加減乗除・剰余・冪乗・シフト・動的値の抽象等価・厳密等価・関係比較・真偽判定、変数・増減文、数値演算・比較、条件・while・後判定・条件分岐、直接関数呼び出しと関数値呼び出し、捕捉ありクロージャ、エラー監視と例外伝播、
