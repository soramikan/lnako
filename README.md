...
`verified` 0件、`trace-confirmed-unattested` 348件、`unverified` 179件です。`native-scalar-system-constants`では17件のglobal readをInterpreter/AOT専用trace・AOT manifest・公式4経路で検証し、`はい`・`いいえ`・`真`・`偽`・`オン`・`オフ`・`NULL`の7件は型付きliteral loweringとして別境界に記録しています。分類やfixtureの存在は、全entryのAOT実行証明を意味しません。
