...
表示、文字列・配列・辞書の添字参照・直接表示、配列・辞書の更新・変数や非配列値からの分割宣言、回数・範囲・変数を含む文字列・配列・辞書の反復、数学38 entry（乱数は固定可能なAOTランタイムPRNGへ接続）、日時29 entry（固定可能なAsia/Tokyo壁時計、Unix秒変換、日時文字列・書式・元号・差分・加算・単調時計へ接続）、URL・Base64 5 entry、パス5 entry（拡張子・終端パス処理）、Nodeホスト情報8 entry（OS・CPU・環境変数取得・一覧取得・カレントディレクトリ取得2 alias・カレントディレクトリ変更2 alias）、`元号データ`（5件の固定元号表）、caniuseの`対応ブラウザ一覧取得`（16キーのv3.7.24生成データ）と`ブラウザ名変換表`（19キーの生成データ）です。数値の`÷÷`は対応し、BigIntの`÷÷`は公式処理系と同じ実行時エラー経路として扱います。非同期処理と残りの標準命令は後続の固定マイルストーンで
