...
`verified` 0件、`trace-confirmed-unattested` 355件、`unverified` 172件です。`native-scalar-system-constants`では17件のglobal readと7件のtyped literalを、Interpreter/AOT専用trace・対応manifest・公式4経路で検証し、`lnako.static-constant-evidence.v2`へ別namespaceの実行証拠として記録しています。分類やfixtureの存在は、全entryのAOT実行証明を意味しません。
