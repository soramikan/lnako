...
実装しています。公式カタログ上は523/527命令がnative、残る4命令が明示的な`compat-js`で、`blocked`は0件です。
各命令を公式差分または副作用を隔離したホストテストへ対応付けています。
...
現段階のAOT対応は、数値・真偽値・null・NULを含むUTF-16文字列・配列と辞書を含む`&`文字列連結・BigInt定数・加減乗除・剰余・冪乗・シフト・動的値の抽象等価・厳密等価・関係比較・真偽判定、変数・増減文、数値演算・比較、条件・while・後判定・条件分岐、直接関数呼び出し、
表示、文字列・配列・辞書の添字参照、配列・辞書の更新・変数や非配列値からの分割宣言、回数・範囲・変数を含む文字列・配列・辞書の反復です。BigIntの`÷÷`、配列・辞書の直接表示、クロージャ、例外、非同期、残りの標準命令は後続の固定マイルストーンで
